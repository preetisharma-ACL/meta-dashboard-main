...
import {
  listDisqualificationBatches,
  revokeDisqualificationBatch,
  fetchDisqualificationAuditLog,
...
// ─── Lead Disqualifications ───────────────────────────────────────────────────
// The record + audit surface for lead-disqualification batches — the CPL-only
// twin of LeadReplacements. Same roles, same layout, minus every money column:
// a disqualification is COUNT ONLY (no cost is credited; the leads simply stop
// being billable, and qualified = generated − replaced − disqualified).
//   • Record  — admin and TIER-1 CMs (canRecordReplacement — one permission
//               covers both lead actions)
...
// key renders "—" rather than a confident 0.
...
export default function LeadDisqualifications() {
  ...
  const [pageSize, setPageSize] = createSignal(
    Number(localStorage.getItem("leadDisqualificationsRowsPerPage")) || 20,
  );
  ...
  const load = async (pageNum = page(), size = pageSize()) => {
    try {
      setLoading(true);
      setError(null);
      const { rows: raw, pagination } = await listDisqualificationBatches({
        page: pageNum,
        pageSize: size,
      });
      ...
      console.error("LeadDisqualifications: load failed", err);
      setError(err?.message || "Could not load disqualification batches.");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const changeRowsPerPage = (size) => {
    ...
    setPageSize(size);
    localStorage.setItem("leadDisqualificationsRowsPerPage", String(size));
    load(1, size);
  };
  ...
  const filtered = createMemo(() => {
    ...
    if (q) {
      data = data.filter((r) =>
        [r.clientName, r.clientEmail, r.projectName, r.notes, r.recordedBy]
          .filter(Boolean)
  ...
  // Roll-up across the rows currently in view. No money column here — a
  // disqualification has no cost, so the panel counts batches and leads only.
  const totals = createMemo(() => {
    const live = filtered().filter((r) => !r.isRevoked);
    return {
      batches: live.length,
      leads: live.reduce((s, r) => s + (Number(r.disqualifiedCount) || 0), 0),
    };
  });
  ...
  const doRevoke = async () => {
    ...
    setRevokeError(null);
    try {
      await revokeDisqualificationBatch(target.id, revokeReason());
      setRevokeTarget(null);
      setRevokeReason("");
      await load(page());
      showToast(
        `Batch #${target.id} revoked — those leads are billable again.`,
        "Disqualification revoked",
      );
    } catch (err) {
  ...
  const openAudit = async (row) => {
    ...
    try {
      setAuditRows(await fetchDisqualificationAuditLog(row.id));
    } catch (err) {
  ...
  // One fewer column than the replacement table (no cost), and the Actions
  // column only exists for an admin — kept in one place so the empty/loading/
  // error rows can never span the wrong width.
  const COLSPAN = () => (canRevokeReplacement() ? 10 : 9);
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 lg:p-8">
      ...
      <div class="flex flex-wrap items-start justify-between gap-3 mb-6">
        <div>
          ...
          <p class="pl-[46px] text-sm text-[#54657E] dark:text-gray-400 mt-0.5">
            Unbillable leads for CPL clients. Qualified leads = generated −
      ...
      <div class="bg-gray-50 dark:bg-gray-800 border border-[#E2E8F1] dark:border-gray-700 rounded-2xl shadow-[0_1px_2px_rgba(16,29,49,.05),0_4px_14px_rgba(16,29,49,.04)] px-5 sm:px-7 py-6 mb-6">
        <div class="flex flex-wrap items-stretch gap-y-5">
          ...
          <div class="px-5 sm:px-7 flex-1 min-w-[170px] border-l border-[#E2E8F1] dark:border-gray-700">
            ...
            <p class="text-xs text-[#54657E] dark:text-gray-400 mt-2">
              removed from the billable count
            </p>
          </div>
      ...
      <div class="flex flex-wrap items-center gap-3 mb-4">
        <input
          ...
          placeholder="Search client, project, notes…"
          ...
          <table class="w-full text-sm">
            <thead class="bg-[#F8FAFC] dark:bg-gray-900/60 border-b border-[#E2E8F1] dark:border-gray-700">
              <tr>
                ...
                <th class={`${TH} text-right`}>Disqualified</th>
                <th class={TH}>Date</th>
            ...
            <tbody>
              <Show
                ...
                fallback={
                  <tr>
                    <td colspan={COLSPAN()} class="p-8 text-center text-[#54657E] dark:text-gray-400">
                      Loading disqualification batches…
                    </td>
                  </tr>
                }
              >
                <Show when={!error()} fallback={
                  ...
                  <Show
                    ...
                    fallback={
                      <tr>
                        <td colspan={COLSPAN()} class="p-8 text-center text-[#54657E] dark:text-gray-400">
                          No disqualification batches recorded yet.
                        </td>
                      </tr>
                    }
                  >
                      ...
                      {(r, i) => (
                        ...
                        >
                          ...
                          <td class="p-3 text-right tabular-nums font-bold text-[#AC2334] dark:text-red-400">
                            {r.disqualifiedCount == null
                              ? "—"
                              : `−${Number(r.disqualifiedCount).toLocaleString("en-IN")}`}
                          </td>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p class="mt-1 text-sm text-[#54657E] dark:text-gray-400">
              The {Number(revokeTarget().disqualifiedCount || 0).toLocaleString("en-IN")}{" "}
              disqualified lead{Number(revokeTarget().disqualifiedCount) === 1 ? "" : "s"} for{" "}
            ...
            <textarea
              ...
              placeholder="Why is this disqualification being reversed?"
