      ...
      <Show when={open()}>
        ...
        <div class="absolute z-20 mt-1 w-full min-w-[220px] bg-white dark:bg-gray-800
          ...
          <ul class="max-h-52 overflow-y-auto">
            <li onClick={() => { props.onChange("all"); close(); }}
            ...
            <For each={visible()}>
              {(opt) => (
                <li onClick={() => { props.onChange(opt.id); close(); }}
                  class={`px-3 py-2 text-sm cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/20
...
export default function Campaigns() {
  ...
  const loadFilterOptions = async () => {
    try {
      ...
      const firstRaw = first.data?.results ?? (Array.isArray(first.data) ? first.data : []);
      const total_pages =
        first.data?.meta?.pagination?.total_pages ??
        first.meta?.pagination?.total_pages ?? 1;
      ...
      setAllAdAccountOptions(
        [...adSeen.entries()].map(([id, name]) => ({ id, name }))
          .sort((a, b) => a.name.localeCompare(b.name))
      );
      setAllClientNomenOptions(
        [...clientSeen.entries()].map(([id, name]) => ({ id, name }))
          .sort((a, b) => a.name.localeCompare(b.name))
      );
    } catch (err) {
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 lg:p-8">
      ...
      <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 mb-4 flex flex-wrap items-center gap-3">
        ...
        <div class="relative flex-1 min-w-[220px]">
          ...
          <input type="text" placeholder="Search by name, project or client…"
            value={search()}
        ...
        <select value={statusFilter()}
          onChange={(e) => applyFilter(setStatusFilter, e.target.value, "status")}
        ...
        <select value={startDatePreset()}
          onChange={(e) => applyFilter(setStartDatePreset, e.target.value, "startDatePreset")}
          class="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700
                 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300
                 focus:outline-none focus:ring-2 focus:ring-purple-400 cursor-pointer">
        ...
        <SearchableSelect
          ...
          onChange={(v) => applyFilter(setClientNomenFilter, v, "clientNomenId")}
        />
        ...
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50
              ...
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("id")}>ID {sortIcon("id")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap min-w-[200px]" onClick={() => toggleSort("name")}>Campaign Name {sortIcon("name")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("client_nomen_name")}>Client Nomen {sortIcon("client_nomen_name")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("project_name")}>Project {sortIcon("project_name")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("ad_account_name")}>Ad Account {sortIcon("ad_account_name")}</th>
              <th class="p-3 text-center cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("status")}>Status {sortIcon("status")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("start_date")}>Start Date {sortIcon("start_date")}</th>
              <th class="p-3 text-left cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("stop_date")}>Stop Date {sortIcon("stop_date")}</th>
              <th class="p-3 text-right cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("cpl")}>CPL {sortIcon("cpl")}</th>
              <th class="p-3 text-right cursor-pointer hover:text-purple-600 whitespace-nowrap text-purple-600" onClick={() => toggleSort("premium_cpl")}>Premium CPL {sortIcon("premium_cpl")}</th>
              <th class="p-3 text-center whitespace-nowrap text-purple-600">Rule</th>
              <th class="p-3 text-right cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("spend")}>Spend {sortIcon("spend")}</th>
              <th class="p-3 text-right cursor-pointer hover:text-purple-600 whitespace-nowrap text-purple-600" onClick={() => toggleSort("premium_spend")}>Premium Spend {sortIcon("premium_spend")}</th>
              <th class="p-3 text-right cursor-pointer hover:text-purple-600 whitespace-nowrap" onClick={() => toggleSort("leads_count")}>Leads {sortIcon("leads_count")}</th>
            </tr>
          </thead>
                ...
                {() => (
                  <tr class="border-b border-gray-100 dark:border-gray-800 animate-pulse">
                    {Array(14).fill(0).map((_, idx) => (
                      <td class="p-3">
                        <div class={`h-3 bg-gray-200 dark:bg-gray-700 rounded ${idx === 1 ? "w-48" : idx === 5 ? "w-16" : "w-20"}`} />
                      </td>
                    ))}
                  </tr>
                )}
                ...
                {(c, i) => {
                  ...
                  return (
                    <tr class={`border-b border-gray-100 dark:border-gray-800 transition-colors
                      ...
                      <td class="p-3 text-gray-600 dark:text-gray-300 font-medium text-sm whitespace-nowrap">{c.id}</td>
                      <td class="p-3 max-w-[220px]">
                        <span class="text-gray-700 dark:text-gray-300 font-medium line-clamp-2" title={c.name}>{c.name}</span>
                      </td>
                      <td class="p-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{c.client_nomen_name ?? "—"}</td>
                      <td class="p-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{c.project_name ?? "—"}</td>
                      <td class="p-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{c.ad_account_name ?? "—"}</td>
                      ...
                      <td class="p-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">{formatDate(c.start_date)}</td>
                      <td class="p-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">{formatDate(c.stop_date)}</td>
                      <td class="p-3 text-right text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">{formatCPL(c.cpl)}</td>
                      <td class="p-3 text-right font-medium whitespace-nowrap">
                        <span class="text-purple-600 dark:text-purple-400">{formatCPL(pm?.cpl)}</span>
                      </td>
                      <td class="p-3 text-center whitespace-nowrap">
                        <Show when={pm?.markup_rule?.label} fallback={<span class="text-gray-400">—</span>}>
                      ...
                      <td class="p-3 text-right text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">{formatCPL(c.spend)}</td>
                      <td class="p-3 text-right font-medium whitespace-nowrap">
                        <span class="text-purple-600 dark:text-purple-400">{formatCPL(pm?.spend)}</span>
                      </td>
                      <td class="p-3 text-right text-gray-700 dark:text-gray-300 whitespace-nowrap">{c.leads_count ?? 0}</td>
                    </tr>
                  );
                }}
              </For>
      ...
      <div class="flex items-center justify-between mt-5 flex-wrap gap-3">
        ...
        <div class="flex items-center gap-2">
          <button
            onClick={() => { const p = page() - 1; setPage(p); loadCampaigns(p); }}
